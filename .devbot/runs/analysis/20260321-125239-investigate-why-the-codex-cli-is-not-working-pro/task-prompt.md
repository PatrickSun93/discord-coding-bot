Act as the `investigator` role in DevBot's `analysis` workflow.
...
Goal: Investigate why the codex CLI is not working properly when called to fix something. Look at:
1. How the codex CLI is invoked/called in the codebase
2. Any error handling or logging around codex calls
3. Configuration files for codex
4. Recent changes that might have broken codex integration
5. Any patterns in how other CLIs are invoked that codex might be missing

Role purpose: Debug root cause before any risky fix.

Workflow stages: reviewer

Selected files: CLAUDE.md, README.md, docs/readme/README.es.md, docs/readme/README.zh-CN.md, CONTRIBUTING.md, devbot-architecture-v4-final.md

Run directory: D:\devitems\discord-coding-bot\.devbot\runs\analysis\20260321-125239-investigate-why-the-codex-cli-is-not-working-pro
